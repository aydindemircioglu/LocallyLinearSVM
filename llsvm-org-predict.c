               ...
               const int jointClustering)
{
    ...
    lineCount = LoadModel(testFile, &values, &arrayCounts, &arrayIndexes, &arrayValues, &targets, NULL, NULL, dim);

	printf("loading test file..\n");
	lineCount = Load(testFile, &values, &arrayCounts, &arrayIndexes, &arrayValues, &targets, NULL, NULL, dim);

	coorIndexes = new int[kNN];
	coorWeights = new double[kNN];
	...
	for(i = 0; i < lineCount; i++)
	{
		...
		CalculateWeights(means, dim, kmeansClusters, arrayValues[i], arrayIndexes[i], arrayCounts[i], coorWeights, coorIndexes, kNN, useDist, distCoef);
	...
	printf("score %s %f\n", trainFile + strlen(FOLDER), correct /(double)lineCount);
