		   ...
		   SolutionInfo* si, int shrinking)
{
	...
	const double time_offset = savetime * TIMEFACTOR;   // convert to seconds, into the future
	
	double starttime = now();
	double nextsavetime = now() + time_offset;
	...
	while(iter < max_iter)
	{
		...
		if (counter % CHECKINTERVAL == 0)
        {
            ...
            {
                // force quit
                printf("Reached walltime, quitting..\n");
                fflush(stdout);
                iter = max_iter + 1;
            ...
            if ((savetime != -1) && (t >= nextsavetime))
            {
                ...
                // create model first
                double q = now();
                double* coeff_ptr = Malloc(double, l);
                hack_model->sv_coef = &coeff_ptr;
                hack_model->SV = Malloc(svm_node*, l);
                int nonzero = 0;
                int positive = 0;
                int negative = 0;
                for (int i=0; i<l; i++)
                ...
                hack_model->rho[0] = calculate_rho();
                
                std::stringstream tmp;
                tmp << "./" << std::setprecision(32) << q << ".libsvm.model";
                svm_save_model(tmp.str().c_str(), hack_model);
                free(coeff_ptr);
                free(hack_model->SV);

                // do not count computation of primal to the save times
                nextsavetime = now() + time_offset;
            }
        }
...
svm_model *svm_load_model(const char *model_file_name)
{
	...
	while(1)
	{
		...
		{		
			...
			result = fscanf(fp,"%d",&param.degree);
		else if(strcmp(cmd,"gamma")==0)
			result = fscanf(fp,"%lf",&param.gamma);
		else if(strcmp(cmd,"coef0")==0)
			result = fscanf(fp,"%lf",&param.coef0);
		else if(strcmp(cmd,"nr_class")==0)
			result = fscanf(fp,"%d",&model->nr_class);
		else if(strcmp(cmd,"total_sv")==0)
			result = fscanf(fp,"%d",&model->l);
		else if(strcmp(cmd,"rho")==0)
		{
			int n = model->nr_class * (model->nr_class-1)/2;
			model->rho = Malloc(double,n);
			for(int i=0;i<n;i++)
				result = fscanf(fp,"%lf",&model->rho[i]);
		}
		else if(strcmp(cmd,"label")==0)
		{
			...
			for(int i=0;i<n;i++)
				result = fscanf(fp,"%d",&model->label[i]);
		}
		else if(strcmp(cmd,"probA")==0)
		{
			...
			for(int i=0;i<n;i++)
				result = fscanf(fp,"%lf",&model->probA[i]);
		}
		else if(strcmp(cmd,"probB")==0)
		{
			...
			for(int i=0;i<n;i++)
				result = fscanf(fp,"%lf",&model->probB[i]);
		}
		else if(strcmp(cmd,"nr_sv")==0)
		{
			...
			for(int i=0;i<n;i++)
				result = fscanf(fp,"%d",&model->nSV[i]);
		}
